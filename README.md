...
- 浅色、深色与跟随系统三种显示模式
- AI 单选试题：动词变形、语境选词、词义、助词、读音、语境理解与近义辨析
- 题目统一控制为 JLPT N3 难度，核心出题词既可能是正确项，也可能作为干扰项
- 当前题显示后可在后台并行预生成 1～3 道储备题，切换下一题时优先即时读取
- 按词持久化 0～100 记忆水平；答对上升、答错下降，并反向影响随机单词与出题概率
...
- `data/学习进度.json`：按词条 ID 保存记忆水平、答题次数、正确数、连续答对与最近复习时间（首次答题时自动创建）。
